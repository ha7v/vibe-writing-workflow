...
1. **对话引导**：通过提问激发用户深入思考
2. **发现关注**：捕捉用户的无意识关注方向
3. **生成知识卡片**：每`4`轮对话自动提炼，或用户任意轮手动触发生成
...
4. **生成初始文档**：`google_web_search` 调研 → 生成3000字学习性文档（格式遵守 [[规范/格式检查#知识卡片格式标准]]）
5. **引导用户阅读初始文档**：
   - 提示用户阅读初始文档
   - 使用以下引导语模板：
...
**步骤1：准备回答**
1. `read_file` 读取项目信息.md获取轮次（绝对轮次、相对轮次）
2. 计算新轮次（绝对+1，相对+1）
3. 标注【第X轮对话】
4. 判断是否触发生成（相对==4）

**步骤2：事实验证（精简版）**
- **仅在涉及以下内容时使用 google_web_search**：
  - 具体数据（数字、参数、数量）
  - 时间节点（年份、日期、历史时间）
- **不需要搜索的问题**：
  - 概念理解（"为什么"、"怎么理解"）
...
**步骤4：更新轮次**
- **关键**：鉴于 `项目信息.md` 通常不大，**允许并推荐直接使用 `write_file` 全量覆盖该文件**。
...
4. **批量创建文件**：逐个 `write_file` 生成`知识卡片/[序号]. [标题].md`
5. **双链批量处理**（见[[规范/双链系统]]）：
   - 一次性 `read_file` 所有源文档（初始文档/已有卡片）
   - 在内存中确定所有新卡片的插入位置
   - 逐个 `replace` 插入双链
6. **更新项目信息**：使用 `write_file` 全量更新，确保准确性。
...
**每轮对话流程**：
- [ ] 步骤1：`read_file` 读取项目信息.md，获取当前轮次
- [ ] 步骤1：计算新轮次（绝对+1，相对+1）
- [ ] 步骤1：标注【第X轮对话】
- [ ] 步骤1：判断是否触发生成（相对==4）
- [ ] 步骤2：判断问题类型，决定是否 `google_web_search`（仅数据/时间）
- [ ] 步骤3：回答+引导性问题（2-3个）
- [ ] 步骤4：**使用 `write_file` 全量更新轮次**（更安全）
- [ ] 来源不明确时，分割线单独询问来源
...
- [ ] **批量处理双链**：
  - 一次性 `read_file` 所有源文档
  - 确定所有卡片的插入位置
  - 逐个 `replace` 插入双链（格式：换行，不空行，使用别名语法）
