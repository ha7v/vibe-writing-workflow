...
- ✅ 补充资料（google_web_search），找到更深刻、更清晰的解释
- ✅ 知识卡片 + 补充资料 → 更好的输出卡片
- ❌ 绝不简化有价值的深刻解释
- ❌ 绝不只是切断知识卡片、改写一下
...
**1. 读取项目全貌**
- `read_file("项目/[项目名]/项目信息.md")` → 获取"待转化知识卡片"列表
- `read_file("项目/[项目名]/初始文档.md")`
- **未转化的知识卡片**（根据待转化列表，使用 `read_file`）
- **已有的输出卡片**（使用 `read_file`）
...
**更新策略**：
- **推荐**：直接使用 `write_file` 全量更新整个 `项目信息.md`，确保准确。
- **替代方案（使用 replace）**：
  - 必须以章节标题 `## 📊 文章结构分析` 作为锚点。
  - 将 `## 📊 文章结构分析` 替换为：
    ```markdown
    ...
    **最近分析**（[日期]）：
    ...
    - 选择方案：接下来写[X]，理由是[...] 
    ```
  - **注意**：这会将旧的分析记录挤下去（如果旧记录在标题下方），或者你需要读取旧内容并手动拼接。**全量更新更安全**。
...
**更新策略**：
- **强烈推荐**：使用 `write_file` **全量覆盖更新**。
- **替代方案（使用 replace - 仅当非常确定锚点时）**：
  - **添加输出卡片**：以 `## ✨ 输出卡片` 为锚点，替换为 `## ✨ 输出卡片\n1. [[输出卡片/主题/01-节点.md|节点标题]] ← [[知识卡片/XX|XX]]` （注意拼接旧列表）。
  - **移除待转化**：找到确切的列表项行（需先 read），替换为空字符串。
  - **安全性提示**：如果无法精确匹配，**必须**回退到全量更新。
...
### 深度分析下一步（工作流1）
- [ ] `read_file` 读取待转化列表 + 未转化的知识卡片 + 已有输出卡片
- [ ] 分析选题核心、已完成内容、最佳方向
- [ ] 提供2-3个具体方案 + AI推荐
- [ ] 使用 `write_file` 全量更新"文章结构分析"（1-2句话）

### 价值点分析 + 生成首版（工作流2）
- [ ] 深度读取知识卡片（所有有价值的深刻解释、类比、例子、细节）
- [ ] 补充资料（`google_web_search`，如需要）
- [ ] 运用叙事手法六要素
- [ ] 目标：激发第二层思考，让用户发现"知识卡片里没想到的问题"
- [ ] 输出卡片必须比知识卡片更好，绝不只是切断改写
- [ ] 格式检查：`read_file` 回读 + `replace` 修正空行
- [ ] 使用 `write_file` 全量更新项目信息.md的3个地方（输出卡片列表、待转化列表、结构分析）
