...
**推荐时**：简单浏览全局
- `read_file("项目/[项目名]/项目信息.md")`（未转化知识卡片 + 已有输出卡片）
- 不深度读取内容，基于标题快速判断
...
**每轮回答格式**：
- a) 深入讲解（基于 `google_web_search` 验证的准确信息，通俗易懂，补充例子）
- b) 提炼2-3个延伸问题（从讲解内容延伸）
- c) 引导："继续问 or 说'整理'"

**事实验证（自动）**：涉及数据/时间/技术规格时 `google_web_search` 验证，内部执行
...
2. **识别当前迭代的卡片**：从对话上下文获取当前卡片的完整路径
3. **检查是否已存在**：`read_file` 读取Canvas，检查这张卡片是否已在白板上（对比file字段）
4. **如果不存在**：添加节点到白板左侧独立位置
   - 路径：从 `项目/` 开始的相对路径（格式：`项目/[项目名]/...`）
   ...
   - **风险提示**：Canvas 是 JSON 格式，直接 `write_file` 生成复杂 JSON 容易出错。建议仅在非常确定的情况下修改。如果文件很大，考虑只输出修改建议，让用户手动添加。
   - 使用 `write_file` 覆盖更新（如果决定自动修改）。
6. **如果已存在**：跳过添加（避免重复）
7. **验证JSON格式**（推荐）：写入后使用 `run_shell_command` (如 `cat ... | grep ...`) 简单验证文件完整性。
...
**用户说"润色"后执行**：
1. `read_file` 读取用户修改后版本（用户删掉的永不恢复）
2. 事实核查（自动）：识别关键事实点 → `google_web_search` 验证 → 输出报告 → 用户确认（如需）
3. **处理【】内容**（如有）：检测到【】→ 自动分析内容 → 扩写语言 → 数据核查 → 直接 `replace` 替换【】
...
**更新策略**：
- **推荐**：使用 `write_file` 全量更新。
- **替代方案（使用 replace）**：
  - 必须以输出卡片的完整行（`1. [[输出卡片/...`）作为锚点。
  - 将其替换为带迭代次数的新行（如 `1. [[输出卡片/...]] (迭代1次) ...`）。
...
**执行流程**：
1. 识别最新文件
2. **事实核查**：读取内容 → 识别关键事实点（数字/专有名词/时间/技术规格）→ `google_web_search` 验证 → 输出报告 → 用户确认（如需）
3. **处理【】内容**（如有）：
   - 检测到【】→ 自动分析内容 → 扩写语言 → 数据核查 → 直接 `replace` 替换【】
4. 识别分割线（`---`上面优化，下面不动）
...
3. **数据核查**：如涉及数据，`google_web_search` 核查真实性，用真实数据替换占位符

4. **自动替换**：直接用扩写内容 `replace` 替换【】及其内容，无需用户确认
...
**执行流程**：
1. `read_file` 读取项目信息.md → 获取待转化知识卡片 + 已有输出卡片（只看标题）
...
**执行流程**：
1. `read_file` 读取Canvas → 获取所有file节点 → 识别已连接/独立卡片
...
4. 用户确认后执行：
   ...
   - 保留用户手动调整（对比项目信息记录）
   - 使用 `write_file` 写入 Canvas 文件
5. 更新项目信息.md的"白板组织记录"（使用 `write_file` 全量更新或以 `## 白板组织` 为锚点的 `replace`）。
6. **验证JSON格式**：写入后验证。
7. 提示用户："白板已更新，可自由调整位置和连接"

**关键规范**：
- **路径**：`glob` 读取路径，确保从 `项目/` 开始，中文标点正确
...
- [ ] 润色阶段：`read_file` 读取修改后版本 + 事实核查 + 处理【】（如有）→ 润色 + 格式检查
- [ ] 更新项目信息.md（推荐全量更新） + 提供选项

**工作流2（润色模式）**：
- [ ] 识别最新文件 → 事实核查 → 处理【】（如有）→ 识别分割线 → 润色 → 格式检查 → 提供选项

**工作流3（轻量推荐）**：
- [ ] `read_file` 读取项目信息.md（只看标题）→ 推荐2-3个方向 → 提供选项

**工作流4（白板组织）**：
- [ ] `read_file` 读取Canvas → 分析关系 → 提供方案 → 纵向排列 + 创建连接线（JSON转义） → 验证JSON格式 → 更新项目信息.md → 提示用户
